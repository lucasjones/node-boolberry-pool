    ...
    for (var minerId in connectedMiners){
        ...
        if (miner.longPoll){
            miner.fetchAddms(function() {
                ...
                miner.longPoll.reply(null, {status: 'OK', jd: miner.getJob(), addms: miner.popAddms()});
                miner.longPoll = null;
            });
        }
        else if (miner.protocol === 'tcp'){
            miner.fetchAddms(function() {
                miner.pushMessage('job', {status: 'OK', jd: miner.getJob(), addms: miner.popAddms()});
            });
        }
    }
}
    ...
    async.waterfall([
        function(cb) {
            ...
            if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
                ...
                cb(null, shareBuffer, hash);
            } else {
                convertedBlob = convertBlockBlob(shareBuffer);
                getFullScratchpad(function(error, result) {
                    if(error) {
                        log('warn', 'Share validator', 'Failed to retrieve scratchpad from daemon: ' + error);
                    } else {
                        var scratchpad = new Buffer(result.scratchpad_hex, 'hex');
                        delete result.scratchpad_hex;
                        var hash = multiHashing.boolberry(convertedBlob, scratchpad, job.height);
                        cb(null, shareBuffer, hash);
                    }
                });
            }
        },
        function(shareBuffer, hash, cb) {
    ...
    switch(method){
        ...
        case 'getjob':
            ...
            if (!longPollingEnabled || miner.protocol === 'tcp'){
                miner.fetchAddms(function () {
                    sendReply(null, {status: 'OK', jd: miner.getJob(), addms: miner.popAddms()});
                });
                return;
            }
            miner.longPoll = {
        ...
        case 'submit':
            ...
            processShare(miner, job, blockTemplate, params.nonce, params.result, function(shareAccepted) {
        ...
        case 'getfullscratchpad':
            ...
            getFullScratchpad(function(error, result) {
                if(error) {
                    sendReply('Failed to retrieve scratchpad');
                    log('warn', componentName, 'Failed to retrieve scratchpad from daemon: ' + error);
                } else {
                    miner.hi = result.hi;
                    sendReply(null, {status: 'OK', hi: result.hi, scratchpad_hex: result.scratchpad_hex});
                }
            });
            break;
        default:
